...
<body>
  ...
  <div class="col-9">
    <%-- <form action="reviewBoardWriteForm" method="POST" enctype="multipart/form-data"> --%>
      ...
      <form:form action="infoBoardWriteModiForm" enctype="multipart/form-data" modelAttribute="board">
        ...
        <tr>
			...
			<td>
				<c:forTokens var="pic" items="${board.board_Pic}" delims="&" varStatus="index">
      				<span id="file_${index.index}"><img class="img" src="/img/${pic}">${pic} <button type="button" class="btn btn-sm btn-outline-danger" onclick="deleteFile('${pic}','${index.index}')">X</button></span>
      			</c:forTokens>
			</td>
...
<!--             <textarea rows="30" cols="75" name="board_Content" ></textarea> -->
        ...
        <tr>
          <td colspan="2"> 
             <button type="button" class="btn btn-outline-danger float-end" onclick="location.href='infoBoardDetail?board_Num=${board.board_Num}&cur_section=${cur_section}&cur_page=${cur_page}'">취소</button>
            <button type="submit" class="btn btn btn-primary float-end" name="infoBoardModi"> 수정</button>
          </td>
        </tr>
      </table>
