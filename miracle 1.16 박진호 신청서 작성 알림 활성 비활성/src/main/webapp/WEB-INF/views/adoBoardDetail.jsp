...
<body>
  <div>	
    <h1>입양홍보페이지 상세보기</h1>
    <div>
      <button type="button" onclick="location.href='applWrite?ado_Num=${info.ado_Num}&ado_AniPic=${info.ado_AniPic}'">입양신청</button> <!-- 입양신청서 이동-->
    </div>
    
		<table> 
			<tr>
				<td>
					<img alt="동물사진" src="${info.ado_AniPic}">
				</td>
			</tr>
			<tr>
				<th>품종 :</th>
			...
			<tr>
				<th>조회수 :</th>
				<td>${info.ado_Views}</td> <!-- 불러온 데이터 -->
			</tr>
      <tr>
      <c:if test="${info.ado_User_Id==curLogin.user_Id}">
      	<td colspan="2">
            <button type="button" onclick="location.href='adoBoardModi?ado_Num=${info.ado_Num}'">수정</button>
      ...
      <tr>
      	<td>
      	<button type="button" onclick="location.href='adoBoardList'">목록으로 돌아가기</button> 
      	</td>
      </tr>
		</table>
		</div>
...
function removeCheck(){
	if(confirm("정말 삭제하시겠습니까??")==true){
		location.href='adoBoardDel?ado_Num=${info.ado_Num}';
	}
	else{
		return false;
	}
