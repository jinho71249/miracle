...
public class CommentController {
	...
	public String commentWrite(@RequestParam("returnNum") int num,
			@RequestParam(value = "path", required = false) String path,
			Comment comment, Model model, Errors errors) {
		...
		if(type.equals("1")) {//입양홍보글일때
			...
			if(path==null) {
				model.addAttribute("path", "adoBoardList");
			}else {
			...
			if(path==null) {
				model.addAttribute("path", "reviewBoard?");
			}else {
				model.addAttribute("path", path);
			}
		}
			...
			if(path==null) {
				model.addAttribute("path", "infoBoardList?");
			}else {
				model.addAttribute("path", path);
			}
		}
			...
			if(path==null) {
				model.addAttribute("path", "freeBoard?");
			}else {
				model.addAttribute("path", path);
			}
		}
	...
	public String commentUpdate(@RequestParam("returnNum") int num,
			@RequestParam(value = "path", required = false) String path,
	...
	public String commentDel(@RequestParam("returnNum") int num, 
							@RequestParam("com_Num") int delNum,
							@RequestParam(value = "path", required = false) String path,
							Model model){     
		String returnPath="main";
		String type=(""+num).substring(0, 1);
